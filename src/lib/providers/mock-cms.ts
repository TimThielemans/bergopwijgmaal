...
function normalizeParser(row: Partial<ParserRecord> | undefined, teamId: string, slug: string): ParserRecord {
  return {
    teamId,
    slug,
    ...(text(row?.volleyClubId) ? { volleyClubId: text(row?.volleyClubId) } : {}),
    ...(text(row?.volleyTeamId) ? { volleyTeamId: text(row?.volleyTeamId) } : {}),
    ...(text(row?.volleySeriesId) ? { volleySeriesId: text(row?.volleySeriesId) } : {}),
